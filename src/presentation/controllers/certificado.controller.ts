...
export class CertificadoController {
  ...
  listByParticipante = async (req: Request, res: Response) => {
    try {
      const useCase = new ListCertificadosByParticipanteUseCase(this.repository)
  ...
  get = async (req: Request, res: Response) => {
    try {
      ...
      if (!result) return res.status(404).json({ error: 'Certificado nao encontrado' })
      return res.json(result)
    } catch (error: any) {
      return res.status(500).json({ error: error.message })
    }
  ...
  remove = async (req: Request, res: Response) => {
    try {
      ...
      if (!deleted) return res.status(404).json({ error: 'Certificado nao encontrado' })
