...
export class AtividadeRepository implements IAtividadeRepository {
  async create(data: Omit<Atividade, 'id'>): Promise<Atividade> {
    ...
    const result = await pool.query(
      'INSERT INTO Atividade (titulo, cargaHoraria, vagas, local, evento_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
  ...
  async findById(id: number): Promise<Atividade | null> {
    const result = await pool.query('SELECT * FROM Atividade WHERE id = $1', [id])
    return result.rows[0] || null
  }

  async findByEventoId(evento_id: number): Promise<Atividade[]> {
    const result = await pool.query('SELECT * FROM Atividade WHERE evento_id = $1', [evento_id])
    return result.rows
  }
  ...
  async update(id: number, data: Partial<Atividade>): Promise<Atividade | null> {
    const { titulo, cargaHoraria, vagas, local, evento_id } = data
    const result = await pool.query(
      'UPDATE Atividade SET titulo=$1, cargaHoraria=$2, vagas=$3, local=$4, evento_id=$5 WHERE id=$6 RETURNING *',
  ...
  async delete(id: number): Promise<boolean> {
    const result = await pool.query('DELETE FROM Atividade WHERE id = $1 RETURNING id', [id])
    return !!result.rows[0]
  }

  async exists(id: number): Promise<boolean> {
    const result = await pool.query('SELECT id FROM Atividade WHERE id = $1', [id])
    return !!result.rows[0]
  }
}
