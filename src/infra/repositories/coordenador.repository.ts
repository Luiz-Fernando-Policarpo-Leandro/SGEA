...
export class CoordenadorRepository implements ICoordenadorRepository {
  ...
  async delete(usuario_id: number): Promise<boolean> {
    const result = await pool.query('DELETE FROM Coordenador WHERE usuario_id = $1 RETURNING usuario_id', [usuario_id])
    return !!result.rows[0]
  }

  async exists(usuario_id: number): Promise<boolean> {
    const result = await pool.query('SELECT usuario_id FROM Coordenador WHERE usuario_id = $1', [usuario_id])
    return !!result.rows[0]
  }
}
