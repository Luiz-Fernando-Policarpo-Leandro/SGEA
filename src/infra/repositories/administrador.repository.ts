...
export class AdministradorRepository implements IAdministradorRepository {
  ...
  async delete(usuario_id: number): Promise<boolean> {
    const result = await pool.query('DELETE FROM Administrador WHERE usuario_id = $1 RETURNING usuario_id', [usuario_id])
    return !!result.rows[0]
  }

  async exists(usuario_id: number): Promise<boolean> {
    const result = await pool.query('SELECT usuario_id FROM Administrador WHERE usuario_id = $1', [usuario_id])
    return !!result.rows[0]
  }
}
