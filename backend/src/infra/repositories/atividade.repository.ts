...
export class AtividadeRepository implements IAtividadeRepository {
  async create(data: Omit<Atividade, 'id'>): Promise<Atividade> {
    ...
    const result = await pool.query(
      ...
      [titulo, tipo, cargaHoraria, vagas, local, horario_inicio, horario_fim, responsavel, evento_id],
    )
...
return result.rows
  ...
  async update(id: number, data: Partial<Atividade>): Promise<Atividade | null> {
    ...
    const result = await pool.query(
      ...
      [titulo, tipo, cargaHoraria, vagas, local, horario_inicio, horario_fim, responsavel, evento_id, id],
    )
    return result.rows[0] || null
  }
