...
export class CertificadoRepository implements ICertificadoRepository {
  async create(data: Certificado): Promise<Certificado> {
    ...
    const result = await pool.query(
      "INSERT INTO certificados (codigo, cargaHoraria, status, participante_id, evento_id) VALUES ($1, $2, $3, $4, $5) RETURNING *",
  ...
  ): Promise<CertificadoWithEvento[]> {
    const result = await pool.query(
      "SELECT c.*, e.nome as evento_nome FROM certificados c JOIN eventos e ON c.evento_id = e.id WHERE c.participante_id = $1",
      [participante_id],
    );
    return result.rows;
  }
  ...
  async update(
    ...
    const result = await pool.query(
      "UPDATE certificados SET cargaHoraria=$1, status=$2, participante_id=$3, evento_id=$4 WHERE codigo=$5 RETURNING *",
  ...
  async delete(codigo: string): Promise<boolean> {
    const result = await pool.query(
      "DELETE FROM certificados WHERE codigo = $1 RETURNING codigo",
      [codigo],
    );
    return !!result.rows[0];
  }
  ...
  async participanteExists(participante_id: number): Promise<boolean> {
    const result = await pool.query(
      "SELECT usuario_id FROM participantes WHERE usuario_id = $1",
      [participante_id],
    );
    return !!result.rows[0];
  }
  ...
  async eventoExists(evento_id: number): Promise<boolean> {
    const result = await pool.query("SELECT id FROM eventos WHERE id = $1", [
      evento_id,
    ]);
    return !!result.rows[0];
  }
