...
export class CoordenadorController {
  ...
  create = async (req: Request, res: Response) => {
    try {
      ...
      if (error.message === 'Coordenador ja cadastrado') return res.status(400).json({ error: error.message })
  ...
  get = async (req: Request, res: Response) => {
    try {
      ...
      if (!result) return res.status(404).json({ error: 'Coordenador nao encontrado' })
      return res.json(result)
    } catch (error: any) {
      return res.status(500).json({ error: error.message })
    }
  ...
  remove = async (req: Request, res: Response) => {
    try {
      ...
      if (!deleted) return res.status(404).json({ error: 'Coordenador nao encontrado' })
